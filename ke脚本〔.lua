...
RunService.Heartbeat:Connect(function()
    ...
    if timeToImpact >= Config.MinImpactTime and timeToImpact <= Config.MaxImpactTime then
        ...
        if ballSpeed > Config.HighSpeedThreshold and timeToImpact < 0.15 then
            ...
            WindUI:Notify({Title = "自动击球", Content = "已开启"})
        else

            _G.AutoSwing = false
            WindUI:Notify({Title = "自动击球", Content = "已关闭"})
        end
    end
})
...
-- 配置参数
local ITEMS_FOLDER = "Items" -- 要监控的文件夹名称
local MAX_DISTANCE = 200 -- 最大显示距离(studs)
local ESP_COLOR = Color3.fromRGB(0, 255, 255) -- 透视颜色(青色)
local TEXT_SIZE = 14 -- 文字大小
local UPDATE_INTERVAL = 0.2 -- 更新间隔(秒)
...
local function createItemESP(item)
    ...
    showNotification("检测到新物品生成: "..item.Name)
    print("物品生成:", item.Name, "位置:", item.Position)
end
...
local function setupItemsMonitoring()
    ...
    if not itemsFolder then
        showNotification("未找到"..ITEMS_FOLDER.."文件夹")
        return
    end
    ...
    showNotification(ITEMS_FOLDER.."文件夹监控已启动")
end
...
startUpdateLoop()
            WindUI:Notify({Title = "透视", Content = "已开启"})
        else

            _G.AutoSwing = false
            WindUI:Notify({Title = "透视", Content = "已关闭"})
        end
    end
})
...
while true do
    ...
    wait(1) -- 每次循环后等待1秒（可根据需要调整）
end

            WindUI:Notify({Title = "自动收集", Content = "已开启 - 按空格键跳跃"})
        else

            _G.InfiniteJump = false
            WindUI:Notify({Title = "自动收集", Content = "已关闭"})
        end
    end
})

Tabs.buyTab = Tabs.MainTab:Tab({ Title = "告诉你个秘密，这个是忍者传奇的🤔", Icon = "zap" })
